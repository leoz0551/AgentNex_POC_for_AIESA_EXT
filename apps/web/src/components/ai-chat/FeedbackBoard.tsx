    ...
    <div className="flex-1 flex flex-col h-full bg-[#f0f1f8] dark:bg-[#0a0a0f] overflow-hidden">
      <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin">
        ...
        <div className="flex items-center gap-8 mb-4">
          <div className="group">
            <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-violet-50 dark:bg-violet-900/10 border border-violet-100 dark:border-violet-800 shadow-sm transition-all hover:scale-105 active:scale-95">
              <span className="text-lg font-bold text-violet-600 dark:text-violet-400">{stats.total}</span>
              <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">Total Feedback</span>
            </div>
          </div>
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-2.5">
              <ThumbsUp className="h-4 w-4 text-slate-400" />
              <span className="text-[13px] font-bold text-slate-400 uppercase tracking-wider">I like</span>
              <span className="text-lg font-black text-slate-700 dark:text-slate-300">{stats.likes}</span>
            </div>
            <div className="flex items-center gap-2.5">
              <ThumbsDown className="h-4 w-4 text-slate-400" />
              <span className="text-[13px] font-bold text-slate-400 uppercase tracking-wider">I don't like</span>
              <span className="text-lg font-black text-slate-700 dark:text-slate-300">{stats.dislikes}</span>
            </div>
            <div className="flex items-center gap-2.5">
              <MessageCircle className="h-4 w-4 text-slate-400" />
              <span className="text-lg font-black text-slate-700 dark:text-slate-300">{stats.comments}</span>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-10 animate-in fade-in duration-700">
            {groupedFeedbacks.map(([dateLabel, items]) => (
              <div key={dateLabel} className="space-y-4">
                <h3 className="text-base font-bold text-slate-400 dark:text-slate-500 tracking-tight pl-2">
                  {dateLabel}
                </h3>
                <div className="space-y-4">
                  {items.map(item => (
                    <div 
                      key={item.id} 
                      className="group flex items-center bg-[#ebedf5] hover:bg-[#e0e3ee] dark:bg-[#1a1b26] dark:hover:bg-[#1f212e] rounded-3xl p-5 shadow-sm transition-all duration-300 border border-transparent hover:border-violet-500/10"
                    >
                      {/* Left: Status Box */}
                      <div className="flex flex-col items-center justify-center w-24 h-20 shrink-0 border-r border-slate-300 dark:border-slate-800/60 pr-5">
                        <div className={`${getStatusColor()} flex flex-col items-center gap-1.5 transition-colors`}>
                      ...
                      <div className="flex-1 px-6 min-w-0">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium text-slate-400 dark:text-slate-500 truncate capitalize">
                            {item.category || 'None'}
                          </span>
                        </div>
                      </div>

                      {/* Right: Metadata - Styled exactly like the user's screenshot */}
                      <div className="flex items-center gap-10 shrink-0 pl-4">
                        <span className="text-[14px] text-slate-400 dark:text-slate-400/90 font-medium">
                          {getRelativeTime(item.timestamp)}
                        </span>
                        <div className="flex items-center gap-2.5">
                          <MessageCircle className="h-[18px] w-[18px] text-slate-400 dark:text-slate-400/80" />
        ...
        {/* Footer info */}
        <div className="pt-12 pb-6 border-t border-slate-200 dark:border-slate-800 text-center">
          <p className="text-[11px] text-slate-400/80 leading-relaxed font-medium">
            ...
            <a href="#" className="underline mx-1 text-slate-400 hover:text-violet-400 transition-colors">User Agreement and Privacy Policy</a> 
            | Language: <span className="text-slate-500 dark:text-slate-400 font-bold">English</span>
          </p>
        </div>
      </div>
    </div>
